...
{
    public class RedisCacheHash : RedisCacheBase, IRedisCacheCollection
    {
        public RedisCacheHash(string redisServer, string password = "", int redisDatabaseId = 0) : base(redisServer, password, redisDatabaseId) { }

        public bool SetCollection<T>(string key, IEnumerable<T> cacheObject, TimeSpan expiryTime) where T : IRedisHashEntry
        {
            RedisDatabase.HashSet(key, cacheObject.Select(itm => itm.ToHashEntry()).ToArray());
